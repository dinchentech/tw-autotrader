...
# 登入並設定專案
gcloud auth login
gcloud config set project 你的專案ID #專案D不是專案名稱google會依你的專案名稱給一個專案ID,但是你可以修改
```

#### 建立 VM + 排程（複製貼上即可,這部分可以讓AI幫你做,但是你的專案必須在google網頁上連接付款帳號否則AI無法安裝）
...
#### 建立本地 Docker Image + 部署到 VM

在**你的 WSL / 本機開發環境**執行：

```bash
# 1. 在專案目錄下，本機 build（快，不用在 e2-micro 上等 pip install）
docker build -t tw-autotrader .

# 2. 將 image pipe 進 SSH，直接載入 VM 的 Docker
...
# 5. 查看日誌
gcloud compute ssh tw-autotrader --zone=asia-east1-b \
  --command="sudo docker logs -f tw_autotrader_bot"
```
...
修改程式後，只需要在**本機**重新 build + SCP 即可更新 VM：
...
# 3. 同步主機檔案（bind mount 需要專案目錄一致）
gcloud compute ssh tw-autotrader --zone=asia-east1-b \
  --command="cd ~/tw-autotrader && git pull"
...
| **部署到 GCP VM** | `docker save tw-autotrader \| gzip -1 \| gcloud compute ssh tw-autotrader --zone=asia-east1-b --ssh-flag="-C" --command="gunzip \| sudo docker load"` |
| **VM 上啟動** | `gcloud compute ssh tw-autotrader --zone=asia-east1-b --command="cd ~/tw-autotrader && sudo docker compose up -d"` |
| **查看 VM 日誌** | `gcloud compute ssh tw-autotrader --zone=asia-east1-b -- "sudo docker logs -f tw_autotrader_bot"` |
