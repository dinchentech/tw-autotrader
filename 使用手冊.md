...
### 4.1 單股交易（`live_trader_finmind.py`）
...
2. 打開瀏覽器，貼上以下網址（**把 `<你的TOKEN>` 換成剛剛複製的那串**）：
   ...
   ```
   ⚠️ **注意**：「bot」必須貼在 Token 前面，中間**不能有空格**。正確範例：
   ```
   https://api.telegram.org/bot1234567890:AAFghijklmnopqrstuvwxyzABCDEFGH/getUpdates
   ```
3. 瀏覽器畫面中搜尋 `"chat":{"id":`，後面那組數字就是 Chat ID
   - 例如個人（你的情況）：`"chat":{"id":8384117171,"first_name":"Frank"...` → Chat ID 就是 `8384117171`
   - 例如群組：`"chat":{"id":-123456789,"title":"群組名"...`
   - **個人通知**：Chat ID 是正數（如 `8384117171`）
   - **群組通知**：Chat ID 是負數（如 `-123456789`）
   - ⚠️ `@你的Bot名稱`（如 `@Hsiehohsuan_bot`）是機器人帳號，不是 Chat ID。Chat ID 是 `/getUpdates` 回傳的 JSON 裡那個**數字**，請不要填 Bot 名稱
...
### 6.6 LINE Notify 通知設定

> ⚠️ 僅 `live_trader_multi.py`（多股多策略版）支援 LINE 通知。  
> `live_trader_finmind.py` 仍以 Telegram 為主。

如果你不需要 LINE 通知，**直接跳過這節就好** — 程式偵測到 `.env` 中沒有 `LINE_NOTIFY_TOKEN` 時會自動跳過 LINE 發送，不影響任何功能。

申請權杖完全不需要寫任何程式碼：

#### 步驟 1：登入 LINE Notify 網站
打開 [LINE Notify 官方網站](https://notify-bot.line.me/)，點擊右上角「登入」，輸入你的 LINE 帳號密碼。

#### 步驟 2：進入個人頁面
登入後，點擊右上角你的 LINE 帳號名稱 → 選單中點「個人頁面」。

#### 步驟 3：發行權杖
在個人頁面往下捲，找到「發行權杖」區塊，點擊綠色「發行權杖」按鈕。

#### 步驟 4：設定名稱與聊天室
| 欄位 | 說明 |
|------|------|
| **權杖名稱** | 通知時顯示的名稱，例如 `台股策略機器人` |
| **接收的聊天室** | 選擇 `透過1對1聊天接收LINE Notify的通知`（個人）或勾選某個群組 |

填完點擊「發行」。

#### 步驟 5：複製權杖
畫面上會出現一串英文與數字組合，這是你的 LINE Notify Token。**按「複製」，它只顯示這一次。**

#### 步驟 6：填入 .env
```env
LINE_NOTIFY_TOKEN=你在步驟 5 複製的那串
```

> ⚠️ **群組通知**：如果步驟 4 選擇發送到群組，發行完畢後，你必須手動打開手機 LINE，進入該群組，將 **LINE Notify** 這個官方帳號邀請加入群組，否則收不到通知。

#### 驗證
`live_trader_multi.py` 會在啟動時自動發送測試通知：
```
🤖 TW AutoTrader 雲端主機已成功啟動！開始全天候監控台股...
```
沒收到請檢查 `.env` 中 `LINE_NOTIFY_TOKEN` 是否正確、群組是否已邀請 LINE Notify 官方帳號。
