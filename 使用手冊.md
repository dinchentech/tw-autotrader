...
# BROKER=esun 後，行情 + 下單全走玉山 API
# ESUN_ENTRY=wss://esuntradingapi.esunsec.com.tw    # 連線入口（向玉山技術客服取得）
# ESUN_ENVIRONMENT=simulation                        # simulation / real
# ESUN_CERT_PATH=/path/to/your/cert.p12              # 憑證檔案
# ESUN_API_KEY=your_esun_api_key
# ESUN_API_SECRET=your_esun_api_secret
# ESUN_ACCOUNT=1234567                               # 證券帳號
# ESUN_ACCOUNT_PASSWORD=                             # 交易密碼（首次自動寫入 keyring）
# ESUN_CERT_PASSWORD=                                # 憑證密碼（首次自動寫入 keyring）
...
| **單股實盤（凱基）** | `python live_trader_finmind.py --symbol 2330 --strategy vwap` |
| **單股實盤（玉山）** | `BROKER=esun python live_trader_finmind.py --symbol 2330 --strategy vwap` |
| **多股多策略實盤（凱基）** | `python live_trader_multi.py` |
| **多股多策略實盤（玉山）** | `BROKER=esun python live_trader_multi.py` |
| **參數調整（回測）** | CLI 命令列 `--參數` |
| **參數調整（實盤）** | 編輯 `.env`，重啟程式 |
