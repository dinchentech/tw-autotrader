...
## 2026-09-06 · 「權值股動能(megacap)」補全輪替權值年缺口（2024）

- 需求：找能跟全輪替互補、在權值股大漲年(2024)也能獲利的策略。全輪替 2024 **-0.8%**（0050 +48.8%），模型C 正常/高穩定 2024 亦負(-7.5%/-0.3%)→ 全沒吃到權值年。
- **新增組 `megacap`（權值股動能）** 到 `scripts/backtest_plan2_monthly_3group.py`（env `MEGA_N`=當時市值前 N 大權值股、挑動能最強、順勢 ma_cross/breakout）。2015-2025(form/addonly/關過熱)：
  - **`MEGA_N=20`：2024 +24.3%**（vs 全輪替 -0.8%、0050 +48.8%）→ **填補全輪替權值年缺口**；全期 +54.5%/年化4.0%/MDD -29.5%。
  - 掃描 MEGA_N=30/50、MOM_DAYS=21/63、TOP_N=3/5：**MEGA_N=20 的 2024 最佳(+24.3%)**；越大越稀釋(金融/電信權值股)、2024 越差。
- **結論**：權值股動能(MEGA_N=20) 正是全輪替的互補腿(權值年+24.3% vs 全輪替-0.8%)；但**到不了 0050 的 +48.8%**——因等權重 top-K(稀釋 AI 領漲股)+每月輪動；若要貼近 0050 需「市值加權/長持」腿。建議作為**並存互補、非單獨**。待混合比例分析(全輪替×權值股動能)。
...
- V4.01 內容：選股工具改讀 `logs/holdings.json`（已持倉保留、只汰換未持倉未選中、keep_wait 不建議賣）；依儀表板驗證「已持倉 4 檔(3653/3008/3017/2059)」保留、3008 維持 breakout；`.env` 現 9 檔/95.1%。
- 驗證：`APP_VERSION=4.01`、`consistency_check` 全通過。

## 2026-09-05 · `.env` 重整：已持倉保留、3008 維持 breakout（被儀表板後續驗證）

- 依儀表板（github.io, 2026-09-03 20:46）「未平倉 = **4 檔**：3653/3008/3017/2059」，確認真正持倉在 VM。修正先前因看本機空 `logs/holdings.json` 而誤把 **3008(大立光 13股)** 移除的錯誤 —— 依「已持倉不動」**加回 3008(breakout)**。
- **`.env` 現 9 檔**：全輪替 keep_wait(3017/3653/2059, 65.1%) + 3008 breakout(5%, 已持倉) + 非輪替 auto 5 檔(2464/4967/2006/3515/2362, 各5%)；總 **95.1%**。已汰換舊固定(2360/6805/2884/2454/3189/6213)與未再選中的舊 auto(1409/6197/2634/6214)；2464 保留。
- **3008 維持 breakout、不改 auto**（使用者 2026-09-05 決定，下月選股日再觀察）。選股工具已改讀 `logs/holdings.json` 判已持倉、保留已持倉/keep_wait、只汰換未持倉未選中。`.env` 為 gitignored 不進 git；同步到 VM 後 live 生效。
