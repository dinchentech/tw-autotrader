...
## 追加案例（2026-08-26 稽核）：README 法人動能 +49.37% 無法重現

- 症狀：2015-2021 窗口重跑 **-4.15%** vs README **+49.37%**；2022-2026-07 接近（+107.31% vs +103.66%）。
- 三層根因（全部在資料層，策略邏輯無誤）：
  1. **TWSE T86 API 僅提供 2017-12-18 後法人資料**（實測 2015/2016 EMPTY）→ 2015-2017 法人全空。修：FinMind 補抓（`fetch_inst_history_bulk`，配額 402 自動等待）。
  2. **回測/實盤共用 `cache/inst_momentum/price/`**：實盤寫入短歷史（yfinance 2021-06 起）覆寫回測長歷史 → 2015-2020 無價格；`min_start` span_ok 例外誤放行。修：回測價格快取獨立 `bt_price/`（`fetch_price_history_bulk`）。
...
- 教訓：**長窗口（5+ 年）回測數字對價格語義極度敏感**；還原價 bug 在短窗口（2022-2026）幾乎無感（+3.65%），在長窗口（2015-2021）造成 +49% → -4% 的翻轉。稽核時務必抽查歷史買入價與真實成交價是否一致。
